...
export class DataController implements IDataController {
  ...
  async importHistory(db: string, key: string) {
    let data = await this.bucket.readXlxs(
      "historie/Communicatierapportage_D&I_FS.xlsx",
    );
  ...
  async entry2(
    ...
    try {
      ...
      switch (topic) {
        case "gemeenten_week":
          data = await this.bucket.readFile(
            year + "/" + week + "/gemeenten_week.csv",
          );
          data = csvToArray(data, ",");
          data = cleanGemeenten(data);
          await this.postgres.bulkInsert(data, "gemeenten_week", db, "db2");
          break;

        case "gemeenten_maand":
          data = await this.bucket.readFile(
            year + "/" + week + "/gemeenten_maand.csv",
          );
          data = csvToArray(data, ",");
          data = cleanGemeenten(data);
          await this.postgres.bulkInsert(data, "gemeenten_maand", db, "db2");
          break;

        case "regelingen_week":
          data = await this.bucket.readFile(
            year + "/" + week + "/regelingen_week.csv",
          );
          data = csvToArray(data, ",");
          cleaned_data = cleanZaaktypes(data);
        ...
        case "regelingen_maand":
          data = await this.bucket.readFile(
            year + "/" + week + "/regelingen_maand.csv",
          );
          data = csvToArray(data, ",");
          cleaned_data = cleanZaaktypes(data);
          await this.postgres.bulkInsert(
