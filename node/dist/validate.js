...
exports.validateCumulativeTotals = void 0;
function validateCumulativeTotals(data) {
        ...
        for (const [baseKey, values] of groupPairs.entries()) {
            ...
            for (let i = 0; i < values.aantal.length; i++) {
                ...
                if (!hasReportedDiscrepancy && Math.abs(runningTotal - values.cumul[i]) > 0.01) {
                    discrepancies.push(Object.assign(Object.assign({}, values.identifiers), { jaar_week: values.jaar_weeks[i], metric: baseKey, week: i + 1, expectedCumul: runningTotal, actualCumul: values.cumul[i], difference: runningTotal - values.cumul[i], weeklyAmount: values.aantal[i] }));
...
exports.validateCumulativeTotals = validateCumulativeTotals;
