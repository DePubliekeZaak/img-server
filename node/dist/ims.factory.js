...
const rowByDesc = (rows, namespace, desc) => {
    return rows.find((r) => r[0] != undefined &&
        r[1] != undefined &&
        r[0].trim() == namespace &&
        r[1].trim() == desc);
};
...
const parseIms = (data, date, week) => {
    ...
    object["pc4"] = "-";
    object["ingediend"] = parseInt(rowByDesc(rows, "ims", "Totaal ingediende aanvragen")[column]);
    ...
    object["imkc_afgeronde_bezwaren"] =
        parseInt(rowByDesc(rows, "imkc", "Afgeronde bezwaren (incl. intrekkingen)")[column]) | 0;
    console.log(object);
    return object;
};
