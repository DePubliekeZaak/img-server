...
const parseIms = (data, date, week) => {
    ...
    console.log(rowByDesc(rows, 'Totaal ingediende bezwaren')[0][53]);
    ...
    object["imsc_zaken"] = parseInt(rowByDesc(rows, 'Aanvragen')[1][column]);
    object["imsc_voorraad"] = parseInt(rowByDesc(rows, 'Voorraad')[0][column]);
    object["imsc_afgehandeld"] = parseInt(rowByDesc(rows, 'Afgehandeld (incl. intrekkingen)')[0][column]);
    object["imsc_verstuurd_besluit"] = parseInt(rowByDesc(rows, 'Toegekend')[0][column]);
    object["imsc_geannuleerd"] = parseInt(rowByDesc(rows, 'Afgewezen')[0][column]);
    object["imsc_besloten_bedrag"] = parseInt(rowByDesc(rows, 'Besloten bedrag')[1][column]);
    object["imsc_ingediende_bezwaren"] = parseInt(rowByDesc(rows, 'Ingediende bezwaren')[0][column]);
    object["imsc_openstaande_bezwaren"] = parseInt(rowByDesc(rows, 'Openstaande bezwaren')[0][column]);
    object["imsc_afgeronde_bezwaren"] = parseInt(rowByDesc(rows, 'Afgeronde bezwaren (incl. intrekkingen)')[0][column]);
    console.log(object);
    return object;
};
