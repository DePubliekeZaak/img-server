...
docker exec -i db1 psql --username "postgres" -d "img" -c "DROP DATABASE IF EXISTS "$DEV" WITH (FORCE);"
docker exec -i db1 psql --username "postgres" -d "img" -c "CREATE DATABASE "$DEV";"
docker exec -i db1 psql --username "postgres" $DEV < /tmp/switch_dump.sql
