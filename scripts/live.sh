...
if [ ! -f .env ]
then
  export $(cat /opt/config/docker/.env | xargs)
fi


docker exec -i db1 pg_dump --username "postgres" $STAGING > /opt/backups/switch_dump.sql
...
docker exec -i db1 psql --username "postgres" $LIVE < /opt/backups/switch_dump.sql
